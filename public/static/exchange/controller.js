...
.filter('sumOfValue', function () {
  ...
  return function(arr, start, end) {
    ...
    var socket = io.connect('https://smarttrex.org',{ 'forceNew': true });
    return {
...
app.controller('CtrMarketHistory', ['$scope','$http','socketio', function($scope,$http,socketio) {
    ...
    socketio.on('Buy_Sell_Matching',(data)=>{
        var data = data[0];
...
app.controller('CtrOrderBuy', ['$scope','$http','socketio', function($scope,$http,socketio) {
    ...
    $http({
        ...
        response.result.reduce(function (res, value) {
            ...
            res[value.price].quantity += parseFloat(value.quantity);
            res[value.price].total += parseFloat(value.total)
            return res;
        }, {});
    ...
    /*socketio.on('OrderBuy:remove', (data)=>{
        var quantity_new,total_new,price_new;
        for (var y = 0; y < data.length; y++) {
            for(var i = 0; i < $scope.data.length; i++) {
                if($scope.data[i].price == data[y].price) {
                    quantity_new = (parseFloat($scope.data[i].quantity)-parseFloat(data[y].quantity)).toFixed(8);
                    total_new = (parseFloat($scope.data[i].total)-parseFloat(data[y].total)).toFixed(8);
                    price_new = $scope.data[i].price;
                    $scope.data.splice(i, 1);
                    if (quantity_new > 0 && total_new > 0) {
                        $scope.data.push({'quantity' : quantity_new,'price' : price_new,'total' : total_new});
    ...
    /*socketio.on('OrderBuy:update', (data)=>{

        console.log(data, $scope.data);
        var quantity_new,total_new,price_new;
        for(var i = 0; i < $scope.data.length; i++) {
            if($scope.data[i].price == data.price) {
                quantity_new = (parseFloat($scope.data[i].quantity)-parseFloat(data.quantity)).toFixed(8);
                total_new = (parseFloat($scope.data[i].total)-parseFloat(data.total)).toFixed(8);
                price_new = $scope.data[i].price;
                $scope.data.splice(i, 1);
                if (quantity_new > 0 && total_new > 0) {
                    $scope.data.push({'quantity' : quantity_new,'price' : price_new,'total' : total_new});
...
app.controller('CtrOrderSell', ['$scope','$http','socketio', function($scope,$http,socketio) {
    ...
    /*socketio.on('OrderSell:remove', (data)=>{
        var quantity_new;
        var total_new;
        var price_new;
        for (var y = 0; y < data.length; y++) {
    ...
    /*socketio.on('OrderSell:update', (data)=>{
        for(var i = 0; i < $scope.data_temp.length; i++) {
            if($scope.data_temp[i]._id == data._id) {
                $scope.data_temp[i].quantity = data.quantity;
                $scope.data_temp[i].total = data.quantity * $scope.data_temp[i].price /100000000;
                break;
            }
        }
        $scope.data = $scope.data_temp;
        $scope.totalItems = $scope.data.length;
    });*/
...
app.controller('CtrOrderOpen', ['$scope','$http', 'socketio', function($scope,$http,socketio) {
    ...
    socketio.on('Buy_Sell_Matching',(data)=>{
        ...
        /*for (var y = 0; y < data.OrderSell_remove.length; y++) {
            for(var i = 0; i < $scope.data.length; i++) {
                if($scope.data[i].price == data.OrderSell_remove[y].price) {
                    quantity_new = (parseFloat($scope.data[i].quantity)-parseFloat(data.OrderSell_remove[y].quantity)).toFixed(8);
                    total_new = (parseFloat($scope.data[i].total)-parseFloat(data.OrderSell_remove[y].total)).toFixed(8);
                    price_new = $scope.data[i].price;
                    $scope.data.splice(i, 1);
        ...
        if (data.OrderSell_remove.length > 0)
        {
            //console.log($scope.data,data.OrderSell_remove);
            for (var y = 0; y < data.OrderSell_remove.length; y++) {
                for(var i = 0; i < $scope.data.length; i++) {
    ...
    /*socketio.on('OrderSell:remove', (data)=>{
        for (var y = 0; y < data.length; y++) {
            for(var i = 0; i < $scope.data.length; i++) {
                if($scope.data[i].remove == data[y]._id+'_Sell') {
                    $scope.data.splice(i, 1);
                    break;
                }
            }
    ...
    socketio.on('OrderBuy:remove', (data)=>{
        for (var y = 0; y < data.length; y++) {
            for(var i = 0; i < $scope.data.length; i++) {
                if($scope.data[i].remove == data[y]._id+'_Buy') {
                    $scope.data.splice(i, 1);
                    break;
                }
            }
...
app.controller('CtrMyMarketHistory', ['$scope','$http','socketio', function($scope,$http,socketio) {
    ...
    /*socketio.on('MatchingOrder', (data)=>{
        for (var i = 0; i < data.length; i++) {
            if (data[i][1] == $scope.user_id || data[i][0] == $scope.user_id)
            {   

                var quantity = data[i][3];
                var price = data[i][4];
                var total = parseFloat(data[i][3])*parseFloat(data[i][4])/100000000;
                var date = data[i][6];
                var type = data[i][5];
                var scope_save = $scope.data;
                scope_save.push({'type' : type ,'quantity' : quantity,'price' : price,'total' : total, 'date' : date , 'user_id_buy' : data[i][1], 'user_id_sell' : data[i][0]});
                $scope.data = scope_save;
                $scope.totalItems = $scope.data.length;
                $scope.setPage = function (pageNo) {
                    $scope.currentPage = pageNo;
