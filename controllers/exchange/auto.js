...
function Create_itemchart(MarketName){
	Chart.find({'MarketName' : MarketName},function(err,result){
		var newChart;
		var today = moment();
		result.length === 0 && (
			newChart = new Chart(),
	        newChart.MarketName = MarketName,
	        newChart.date =  moment(today).format(),
	        newChart.save( (err) => {
	        	create_chart_test(MarketName);
	        })
		)
	})
}
